	...
	while ((sock_gres = (sock_gres_t *) list_next(sock_gres_iter))) {
		...
		*max_tasks_this_node = (*max_tasks_this_node / task_cnt_incr) *
		...
		if (*max_tasks_this_node < *min_tasks_this_node) {
			error("%s: min_tasks_this_node:%d > max_tasks_this_node:%d",
			      __func__,
			      *min_tasks_this_node,
			      *max_tasks_this_node);
		...
		if (mc_ptr->cpus_per_task) {
			...
			while (*max_tasks_this_node >= *min_tasks_this_node) {
				...
				if (req_cores <= avail_cores_tot) {
					if (removed_tasks)
						log_flag(GRES, "%s: settings required_cores=%d by max_tasks_this_node=%d(reduced=%d) cpus_per_task=%d cpus_per_core=%d threads_per_core:%d",
							 __func__,
							 req_cores,
							 *max_tasks_this_node,
							 removed_tasks,
		...
		if (cpus_per_gres) {
			if (job_specs->gres_per_node) {
				...
				i = job_specs->gres_per_task *
				    *max_tasks_this_node;
				log_flag(GRES, "%s: estimating req_cores max_tasks_this_node=%d gres_per_task=%"PRIu64,
					 __func__,
					 *max_tasks_this_node,
					 job_specs->gres_per_task);
